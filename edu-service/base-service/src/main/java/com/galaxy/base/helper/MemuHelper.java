...
public class MemuHelper {
     ...
     * @return
     */
    public static List<JSONObject> bulid(List<EduMenu> treeNodes) {
        List<JSONObject> meuns = new ArrayList<>();
        if(treeNodes.size() == 1) {
            EduMenu topNode = treeNodes.get(0);
            //左侧一级菜单
            List<EduMenu> oneMeunList = topNode.getChildren();
            for(EduMenu one :oneMeunList) {
                ...
                List<EduMenu> twoMeunList = one.getChildren();
                for(EduMenu two :twoMeunList) {
                    ...
                    List<EduMenu> threeMeunList = two.getChildren();
                    for(EduMenu three :threeMeunList) {
